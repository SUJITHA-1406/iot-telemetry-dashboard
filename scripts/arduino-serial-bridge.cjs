 ...
 * 
 * Usage:
 *   node scripts/arduino-serial-bridge.cjs [COM_PORT] [BAUD_RATE]
 *   Example: node scripts/arduino-serial-bridge.cjs COM3 115200
 * 
...
function parseLine(line) {
  ...
  if (tempMatch || humMatch || moistMatch) {
    const temp = tempMatch ? parseFloat(tempMatch[1]) : 32.2;
    const hum = humMatch ? parseFloat(humMatch[1]) : 56.1;
    const moist = moistMatch ? parseFloat(moistMatch[1]) : 29;
    ...
    return {
      ...
      humidity: hum,
      moisture: moist,
      status: statusVal || (moist < 22 ? "WARNING" : "NORMAL"),
      created_at: new Date().toISOString()
    };
  }
